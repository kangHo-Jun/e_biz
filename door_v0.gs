...
function 계산_영림발주서_가격_내부() {
  ...
  for (var i = 시작행; i <= 끝행; i++) {
    ...
    if (계산성공) {
      ...
      // 단, 22행~34행은 제외
      if (ba값 && ba값.toString().trim() !== "" && ba값.toString().trim() !== "없음") {
         if (i >= 22 && i <= 34) {
...
function onEdit(e) {
  try {
    ...
    if (sheet.getName() === "영림발주서") {
       ...
       // (그래야 나중에 '단가계산' 눌렀을 때 다시 계산됨)
       if (row >= 22 && row <= 34) {
          if (col === 1 || col === 49) { // A열 or AW열
             sheet.getRange(row, 1).clearNote();
          }
       }
       
       // B. 색상 자동 연동 (AW열 입력 시 → AX열 자동 입력)
       if (col === 49) {
          // 행 범위 체크 (12행 ~ 35행)
          if (row >= 12 && row <= 35) {
             ...
             if (jsonMap) {
               ...
               if (result) {
                 sheet.getRange(row, 50).setValue(result);
                 e.source.toast("색상 자동완성: " + result, "성공");
                 axValue = result;
               } else {
                 e.source.toast("색상 매칭 실패: [" + inputKey + "]", "알림");
               }
             } else {
                e.source.toast("데이터 없음. 업데이트 필요", "오류");
             }
          
          // 2. BA열 품목 정보 자동 입력 (신규 로직)
          ...
          // 22행~34행만 적용 (문짝)
          if (row >= 22 && row <= 34) {
              var avValue = sheet.getRange(row, 48).getValue(); // AV열 (48번째 열)
              var awValue = sheet.getRange(row, 49).getValue(); // AW열
              var axValue = sheet.getRange(row, 50).getValue(); // AX열
              var aqCell = sheet.getRange(row, 43); // AQ열 (43번째 열)
              ...
              if (avNum <= 2165) {
                 // 조건3: AV <= 2165 → AQ = 빈칸
                 aqCell.setValue("");
                 Logger.log("  [AQ 자동입력] 행 " + row + ": AV=" + avNum + " <= 2165 → AQ=빈칸");
              } else {
                 // AV >= 2166: AG/AH 매칭 확인
                 var testSheet = e.source.getSheetByName("테스트");
                 if (!testSheet) {
                    Logger.log("  [AQ 자동입력] 테스트 시트를 찾을 수 없음");
                    return;
                 }
                 
                 // AG, AH 열 데이터 로드 (AG=33, AH=34)
                 var agData = testSheet.getRange("AG1:AG300").getValues();
                 var ahData = testSheet.getRange("AH1:AH300").getValues();
                 
                 // AW, AX 값 준비 (대소문자 구분 없음)
                 var searchKeys = [];
                 if (awValue && awValue.toString().trim() !== "") {
                    searchKeys.push(awValue.toString().trim().toUpperCase());
                 }
                 if (axValue && axValue.toString().trim() !== "") {
                    searchKeys.push(axValue.toString().trim().toUpperCase());
                 }
                 
                 if (searchKeys.length === 0) {
                    Logger.log("  [AQ 자동입력] 행 " + row + ": AW/AX 값 없음");
                    return;
                 }
                 ...
                 for (var i = 0; i < ahData.length; i++) {
                    var cellValue = ahData[i][0];
                    if (!cellValue) continue;
                    var cellUpper = cellValue.toString().toUpperCase();
                 ...
                 // AG 열 매칭 확인 (AH에서 매칭 안 된 경우만)
                 if (!matchedAH) {
                    for (var i = 0; i < agData.length; i++) {
                       var cellValue = agData[i][0];
                       if (!cellValue) continue;
                       var cellUpper = cellValue.toString().toUpperCase();
                 ...
                 if (matchedAH) {
                    aqCell.setValue("Y");
                    e.source.toast("AQ 자동입력: Y (AH 매칭)", "성공");
                    Logger.log("  [AQ 자동입력] 행 " + row + ": AV=" + avNum + ", AH 매칭 → AQ=Y");
                 } else if (matchedAG) {
                    aqCell.setValue("N");
                    e.source.toast("AQ 자동입력: N (AG 매칭)", "성공");
                    Logger.log("  [AQ 자동입력] 행 " + row + ": AV=" + avNum + ", AG 매칭 → AQ=N");
                 } else {
                    // 매칭 실패 시 빈칸 유지 또는 기존 값 유지
                    Logger.log("  [AQ 자동입력] 행 " + row + ": AV=" + avNum + ", 매칭 실패");
                 }
              }
          }
       }
    } // Close if (col === 49)
  } // Close if (sheet.getName() === "영림발주서")
...
function updateColorCodeMap() {
  ...
  try {
    ...
    debugLog.push("총 시트 개수: " + sheets.length);
    
    for (var s = 0; s < sheets.length; s++) {
      var sheet = sheets[s];
      var data = sheet.getRange("V1:Z150").getValues();
      ...
      for (var r = 0; r < data.length; r++) {
        for (var c = 0; c < data[r].length; c++) {
          ...
          if (text.includes("107")) {
             debugLog.push("✅ [" + sheet.getName() + "] 시트에서 '107' 발견: " + text);
             foundSheetName = sheet.getName();
             sheetHasData = true;
          }
          ...
          if (code1 && code2) {
            ...
            colorMap[code2] = code1;
            
            var code2NoSpace = code2.replace(/\s+/g, '');
            if (code2 !== code2NoSpace) {
               colorMap[code2NoSpace] = code1;
            }
            if (sheetHasData) {
               colorCount++;
            }
    ...
    // ---------------------------------------------------------
    // 2. 로컬 '테스트' 시트 품목 정보 처리 (신규 로직)
    // M~O열, P~R열, S~U열 스캔
    // ---------------------------------------------------------
    var localSS = SpreadsheetApp.getActiveSpreadsheet();
    var testSheet = localSS.getSheetByName("테스트");
    var infoMap = {};
    var infoCount = 0;

    if (testSheet) {
      // M1:U300 읽기 (총 9열: M,N,O, P,Q,R, S,T,U)
      // 인덱스: M=0, N=1, O=2 | P=3, Q=4, R=5 | S=6, T=7, U=8
      var rangeData = testSheet.getRange("M1:U300").getValues();
      
      // [수정] 각 그룹별로 별도 맵 생성 (중복 제품명 문제 해결)
      var infoMapM = {}; // M열 제품 (O열 상태)
      var infoMapP = {}; // P열 제품 (R열 상태)
      var infoMapS = {}; // S열 제품 (U열 상태)
      
      // 3개 그룹을 각각 순회
      var groups = [
        {keyIdx: 0, valIdx: 1, stsIdx: 2, group: 'M', map: infoMapM}, // M, N, O
        {keyIdx: 3, valIdx: 4, stsIdx: 5, group: 'P', map: infoMapP}, // P, Q, R
        {keyIdx: 6, valIdx: 7, stsIdx: 8, group: 'S', map: infoMapS}  // S, T, U
      ];
      ...
      for (var i = 0; i < rangeData.length; i++) {
        ...
        for (var g = 0; g < groups.length; g++) {
          var grp = groups[g];
          var key = row[grp.keyIdx];   // 검색 키 (예: PSM195 소프트매트...)
          var val = row[grp.valIdx];   // 출력 값 (예: 캐시미어 or 값)
          var status = row[grp.stsIdx]; // 상태 (단종 여부) - 해당 그룹의 상태 열만 사용

          if (key && key.toString().trim() !== "") {
             var keyStr = key.toString().trim();
             var statusStr = status ? status.toString().trim() : "";
             var valStr = val ? val.toString().trim() : "";
             
             // 정보 저장 객체
             var isDiscontinued = (statusStr === "단종");
             var isScheduled = (statusStr === "단종예정");

             var infoObj = {
               value: valStr,
               isDiscontinued: isDiscontinued,
               isScheduled: isScheduled,
               statusText: statusStr, // 원본 상태 텍스트 저장
               fullText: keyStr,
               group: grp.group // 어느 그룹(M/P/S)에서 왔는지 저장
             };
             
             // [수정] 각 그룹의 맵에 별도로 저장 (덮어쓰기 방지)
             grp.map[keyStr] = infoObj;
             infoCount++;
          }
        }
      }
      
      // [신규] 3개 맵을 하나의 객체로 통합하여 저장
      infoMap = {
        M: infoMapM,
        P: infoMapP,
        S: infoMapS
      ...
      debugLog.push("⚠️ 로컬에 '테스트' 시트가 없습니다. 품목 정보 업데이트 실패.");
    }
    

    // 저장
    var props = PropertiesService.getScriptProperties();
    props.setProperty("COLOR_MAP", JSON.stringify(colorMap));
    props.setProperty("PRODUCT_INFO_MAP", JSON.stringify(infoMap));
    
    var msg = "";
    if (foundSheetName) {
      msg = "✅ [외부] 색상 데이터 업데이트: " + colorCount + "쌍\n";
    } else {
      msg = "❌ [외부] '영림107' 데이터 찾기 실패\n";
    }
    
    if (testSheet) {
       msg += "✅ [로컬] 품목 정보 업데이트: " + infoCount + "건\n";
    } else {
       msg += "❌ [로컬] '테스트' 시트 없음\n";
    }
